...
func TestDownloadFile(t *testing.T) {
	...
	h := NewHTTPCache(c, testutils.NewSilentLogger(), testutils.NewSilentLogger(), true, false, false, false, "sha256", "", "", math.MaxInt64)
...
func TestUploadEmptyActionResult(t *testing.T) {
	...
	h := NewHTTPCache(c, testutils.NewSilentLogger(), testutils.NewSilentLogger(), validate, mangle, checkClientCertForReads, checkClientCertForWrites, "sha256", "", "", math.MaxInt64)
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(h.CacheHandler)
	handler.ServeHTTP(rr, r)
...
func TestRemoteReturnsNotFound(t *testing.T) {
	...
	h := NewHTTPCache(emptyCache, testutils.NewSilentLogger(), testutils.NewSilentLogger(), true, false, false, false, "sha256", "", "", math.MaxInt64)
...
func TestManglingACKeys(t *testing.T) {
	...
	h := NewHTTPCache(diskCache, testutils.NewSilentLogger(), testutils.NewSilentLogger(), false, true, false, false, "sha256", "", "", math.MaxInt64)
	// create a fake http.Request
	data, hash := testutils.RandomDataAndHash(blobSize)
